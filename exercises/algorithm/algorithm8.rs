...
impl<T> myStack<T> {
    ...
    pub fn pop(&mut self) -> Result<T, &str> {
        // Check if the stack (queue q1) is empty
        if self.q1.is_empty() {
        ...
        while self.q1.size() > 1 {
            // Try to dequeue from q1 and enqueue to q2
            if let Ok(elem) = self.q1.dequeue() {
                self.q2.enqueue(elem);
            } else {
                return Err("Failed to dequeue from q1");
            }
        }

        // Dequeue the last element, which is the top of the stack
        let result = self.q1.dequeue().map_err(|_| "Failed to dequeue from q1")?;
